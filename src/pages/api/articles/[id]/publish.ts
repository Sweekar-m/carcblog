...
export const POST: APIRoute = async ({ locals, params }) => {
  ...
  const { article } = authResult;

  if (article?.status === 'archived') {
    return errorResponse('Cannot publish an archived article. Unarchive or create a new draft first.', 400);
  }
  ...
  try {
    ...
    return errorResponse('Failed to publish article', 500, err);
  }
};
