...
import "./chunks/astro-designed-error-pages_Nj8Ot2at.mjs";
import "cookie";
import { s as sequence } from "./chunks/render-context_C-Wna52o.mjs";
async function createAsyncLocalStorage() {
  const { AsyncLocalStorage } = await import("node:async_hooks");
  return new AsyncLocalStorage();
}
const authAsyncStorage = await createAsyncLocalStorage();
const __vite_import_meta_env__ = { "ASSETS_PREFIX": void 0, "BASE_URL": "/", "DEV": false, "MODE": "production", "PROD": true, "PUBLIC_CLERK_PUBLISHABLE_KEY": "pk_test_cHJvYmFibGUtc3BvbmdlLTcyLmNsZXJrLmFjY291bnRzLmRldiQ", "PUBLIC_CLERK_SIGN_IN_URL": "/auth/sign-in", "PUBLIC_CLERK_SIGN_UP_URL": "/auth/sign-up", "PUBLIC_SUPABASE_ANON_KEY": "sb_publishable_FuBvSFZ42xLpkfePmU5lng_vi0VCYbB", "PUBLIC_SUPABASE_URL": "https://otflzhkcerzmaibmzgao.supabase.co", "SITE": "https://carcblog.com", "SSR": true };
...
function getContextEnvVar(envVarName, contextOrLocals) {
  ...
  return Object.assign(__vite_import_meta_env__, { CLERK_SECRET_KEY: "sk_test_908d2i4uDnyFoDtJxds0yvG4sfyeFn4rjYyH7aZJf7", PUBLIC: process.env.PUBLIC })[envVarName] || void 0;
}
...
const createClerkClientWithOptions = (context, options) => createClerkClient({
  ...
  userAgent: `@clerk/astro@3.4.18`,
  sdkMetadata: {
    name: "@clerk/astro",
    version: "3.4.18",
    environment: Object.assign(__vite_import_meta_env__, { CLERK_SECRET_KEY: "sk_test_908d2i4uDnyFoDtJxds0yvG4sfyeFn4rjYyH7aZJf7", PUBLIC: process.env.PUBLIC }).MODE
  },
  telemetry: {
...
function decorateRequest(locals, res) {
  if (res.headers.get("content-type") === "text/html") {
    ...
    const clerkAstroData = encoder.encode(`<script id="__CLERK_ASTRO_DATA__" type="application/json">${JSON.stringify(locals.auth())}<\/script>
`);
    const clerkSafeEnvVariables = encoder.encode(`<script id="__CLERK_ASTRO_SAFE_VARS__" type="application/json">${JSON.stringify(getClientSafeEnv(locals))}<\/script>
`);
